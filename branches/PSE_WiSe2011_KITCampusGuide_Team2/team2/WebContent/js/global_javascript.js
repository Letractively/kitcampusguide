...
function resize_window() {
	document.getElementById('wrapper').style.height = parseInt(document.body.clientHeight)
			+ 'px';
}
...
function noSpam() {
	...
	for ( var i = 0; i < a.length; i++) {
		if ((a[i].href.search(/emailform\b/) != -1)
				&& (a[i].className.search(/force_form\b/) == -1)) {
...
function remove_liststyle() {
	if (document.getElementById("right-row")) {
		var lis = document.getElementById("right-row").getElementsByTagName(
				"li");
...
function collapseFAQ() {
	...
	for (i = 0; i < spans.length; i++) {
		if (spans[i].className == 'faq_question') {
			...
			counter_question++;
			spans[i].onclick = new Function(
					"document.getElementById(this.id + '_answer').style.display = (document.getElementById(this.id + '_answer').style.display == 'none') ? 'block' : 'none';");
