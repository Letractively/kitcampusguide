...
public class ReservationControllerTest {
	...
	public void testDeleteReservation() {
		...
		// check if reservation was deleted properly
		testListReservations();

		// check res notf ound
		// check if res belongs to another user
	}
	...
	private ReservationModel updateReservation(String reservationID, Date endTime, int wpCount) {
		...
		// Create reservation model with values of updated reservation
		...
		BindingResult updatedReservationResult = new BeanPropertyBindingResult(updatedReservation,
				"updatedReservation");

		// update
		String view = reservationController.updateReservation(model, reservationID, updatedReservation,
				updatedReservationResult);

		// check if view is correct and no form errors have occurred
		assertEquals("reservation/details", view);
		assertFalse(model.containsAttribute("formErrors"));

		Map<String, Object> modelAttributes = model.asMap();

		// Checks on Reservation
		assertTrue(modelAttributes.get("reservation") instanceof ReservationModel);
		ReservationModel resModel = (ReservationModel) modelAttributes.get("reservation");
	...
	public void testUpdateEndTime() {
		...
		ReservationModel resModel = updateReservation(resUser1.getId(), newEndTime, resUser1
	...
	public void testUpdateRoomReservation() {
		...
		// create a new reservation with one room
	...
	public void testUpdateWrongValues() {
		...
		// Create mock models and binding result for the test
		Model model = new BindingAwareModelMap();
		...
		String view = reservationController.updateReservation(model, resUser1.getId(), updatedReservation,
				updatedReservationResult);
		assertEquals("reservation/details", view);

		// Check if wrong values were detected
	...
	private void testUpdateReservation(String wrongReservationID) {
		...
		String view = reservationController.updateReservation(model, wrongReservationID, updatedReservation,
				updatedReservationResult);
